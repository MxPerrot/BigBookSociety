...
async function fetchUserData() {
    try {
        const response = await fetch(`${API_PATH}/users/me`, {
            method: "GET",
            headers: {
        ...
        console.log(user.group_role == "admin");

        if (user.group_role == "admin") {
            is_admin = `<li><a href="../html/admin.html">ADMINISTRATION</a></li>
                        <li><a class="deconnexion" href="javascript:void(0);" onclick="localStorage.clear();refreshCarrousel()">DÉCONNEXION</a></li>`;
        } else {
            is_admin = `<li><a class="deconnexion" href="javascript:void(0);" onclick="localStorage.clear();refreshCarrousel()">DÉCONNEXION</a></li>`;
        }
    ...
    if (headerElement && headerElement.shadowRoot) {
        ...
        navLinks.forEach(li => {
            ...
            if (link && link.getAttribute("href") === "../html/meslivres.html" && !token) {
                li.innerHTML = `<li id="deroulant-livre"><a href="../html/connexion.html">MES LIVRES</a></li>`;
            }
        });
    }
});