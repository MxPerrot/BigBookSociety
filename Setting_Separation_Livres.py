...
#Création dataframe pour la table setting
...
# Création dataframe pour le lien entre les livres et leur setting
settingLinkData = settingData[['id_cadre','id_livre']]
settingLinkData = settingLinkData.drop_duplicates()
settingLinkData.to_csv("./SQL/livre_cadre.csv", index=False)

settingData = settingData.drop(columns=['id_livre'])
settingData.to_csv("./SQL/cadre.csv", index=False)

#Création dataframe pour la table setting
booksData = books[['id','title','rating_count','review_count','average_rating','five_star_ratings','four_star_ratings','three_star_ratings','two_star_ratings','one_star_ratings','number_of_pages','date_published','original_title','isbn','isbn13','description']]
