...
CREATE OR REPLACE FUNCTION updateNote() RETURNS TRIGGER AS $$
  DECLARE 
    ...
    nom_note TEXT;
    old_nom_note TEXT;
    sql_query TEXT;  -- Déclaration correcte de la variable sql_query
  BEGIN
    IF NEW.note IS NOT NULL THEN
      -- Vérifie si la note existe déjà pour l'utilisateur
      ...
      ELSIF NEW.note = 5 THEN
          nom_note := 'nb_note_5_etoile';
      END IF;

      -- Si la note n'existe pas encore, on ajoute une nouvelle note
      ...
      ELSE
          -- Si la note existait déjà, on met à jour les anciennes et nouvelles notes
          IF OLD.note = 1 THEN
